...
public class OpenAPIConnectorTestBotCreation {
	...
	// assertEquals("object", function.getContentType());
	// assertFalse(function.isRequired());
	// assertNotNull(function.getChildAttributes());
	// assertEquals(2, function.getChildAttributes().size());
	// Iterator<ServiceFunctionAttribute> citer =
	// function.getChildAttributes().iterator();
	...
	assertEquals(ParameterType.CHILD, type.getParameterType());
