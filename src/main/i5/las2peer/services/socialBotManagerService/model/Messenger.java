...
public class Messenger {
  ...
  public Messenger(
    ...
    if (chatService.contentEquals("Slack")) {
      ...
      this.chatMediator =
        new RocketChatMediator(token, database, new RasaNlu("rasaUrl"));
  ...
  public void setContextToBasic(String channel) {
    ...
    if (state != null) {
      if (state.getFollowingMessages() == null) {
        ...
        this.chatMediator.sendMessageToChannel(
            channel,
            state.getResponse(random).getResponse()
          );
      } else {}
    }
  }
    ...
    for (ChatMessage message : newMessages) {
      try {
        ...
        if (message.getText().startsWith("!")) {
          ...
          System.out.println(
            message.getFileName() + " + " + message.getFileBody()
          );
          if (
            bot.getRasaServer(currentNluModel.get(message.getChannel())) != null
          ) {
            intent =
              bot
                .getRasaServer(currentNluModel.get(message.getChannel()))
          ...
          if (intent.getConfidence() >= 0.40 || message.getFileName() != null) {
            if (state == null) {
              if (message.getFileName() != null) {
                ...
                if (
                  this.knownIntents.get(intent.getKeyword()) != null &&
              ...
              if (
                state.getFollowingMessages() == null ||
                ...
                state.getFollowingMessages().get(intent.getKeyword()) != null
              ) {
                ...
                if (message.getFileBody() != null) {
                  if (
                    ...
                    state =
                      state.getFollowingMessages().get(intent.getKeyword());
                    stateMap.put(message.getChannel(), state);
                  } else {
                    state = this.knownIntents.get("default");
                  }
                ...
                System.out.println(
                  intent.getKeyword() +
                  " not found in state map. Confidence: " +
                  intent.getConfidence() +
                  " confidence."
                );
                // try any
            ...
            if (intent.getEntitieValues().size() == 1) {
              for (ChatResponse res : state.getResponseArray()) {
                ...
                if (
                  res
                    .getTriggerEntity()
                    .equals(intent.getEntitieValues().get(0))
                ) {
                  response = res;
                }
              }
              ...
              if (response.getResponse() != "") {
                ...
                if (!response.getFileURL().equals("")) {
                  ...
                  try {
                    ...
                    String urlEmail = response
                      .getFileURL()
                      .replace("menteeEmail", message.getEmail());
                    ...
                    httpConn.addRequestProperty(
                      ...
                      Base64
                        .getEncoder()
                        .encodeToString(
                          (bot.getName() + ":actingAgent").getBytes()
                        )
                    );

                    String fieldValue = httpConn.getHeaderField(
                      "Content-Disposition"
                    );
                    System.out.println(fieldValue);
                    if (
                      fieldValue == null || !fieldValue.contains("filename=\"")
                    ) {
                    ...
                    if (!fieldValue.equals("pdf.pdf")) {
                      ...
                      if (
                        urlEmail.contains(".pdf") ||
                        urlEmail.contains(".png") ||
                        urlEmail.contains(".svg") ||
                        urlEmail.contains(".json") ||
                        urlEmail.contains(".txt")
                      ) {
                        fileName =
                          urlEmail.split("/")[urlEmail.split("/").length - 1];
                      }
                    }
                    ...
                    int bytesRead;
                    while (
                      (bytesRead = in.read(dataBuffer, 0, file_size)) != -1
                    ) {
                      fileOutputStream.write(dataBuffer, 0, bytesRead);
                    }
                    fileOutputStream.close();
                    this.chatMediator.sendFileMessageToChannel(
                        message.getChannel(),
                        new File(fileName),
                        ""
                      );
                    ...
                    System.out.println(
                      "Could not extract File for reason " + e
                    );
                ...
                if (response.getTriggeredFunctionId() != null) {
                  this.triggeredFunction.put(
                      message.getChannel(),
                      response.getTriggeredFunctionId()
                    );
                  contextOn = true;
                }
              } else {
