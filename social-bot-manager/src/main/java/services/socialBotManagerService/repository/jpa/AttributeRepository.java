...
public interface AttributeRepository extends JpaRepository<Attributes, Long> {
    ...
    Long findIdByBot(@Param("bot") String bot, @Param("channel") String channel, @Param("username")String username, @Param("key") String key);   
    ...
    @Query(value = "SELECT at FROM Attributes at WHERE at.username = :username AND at.bot = :bot")
    Attributes findAttributeByUser(@Param("username")String username, @Param("bot") String bot);
}
